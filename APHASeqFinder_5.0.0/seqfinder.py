...
def get_tool_version(name):
    ...
    try:
        ...
        try:
            ...
            version_output = result.stdout.decode('latin1').strip()  # Try a different encoding

        # Parsing the version from the output
        if name == 'abricate':
            version = version_output.split()[1]
        elif name == 'makeblastdb':
            version = version_output.split(" ")[1].split("\n")[0]
        elif name == 'mlst':
            version = version_output.split()[1]
        elif name == 'samtools':
...
tools = ['makeblastdb', 'mlst', 'abricate', 'samtools', 'bcftools']
required_versions = {'mlst': '2.23.0', 'abricate': '1.0.1', 'makeblastdb': '2.15.0+', 'samtools': '1.20', 'bcftools': '1.20'}
required_versions_install = {'mlst': '2.23.0', 'abricate': '1.0.1', 'blast': '2.15.0', 'samtools': '1.20', 'bcftools': '1.20'}
tool_to_package = {'makeblastdb': 'blast', 'mlst': 'mlst', 'abricate': 'abricate', 'samtools': 'samtools', 'bcftools': 'bcftools'}
...
if not_installed_tools or incorrect_version_tools:
    ...
    install_commands = ' '.join([f"{tool_to_package[tool]}={required_versions_install[tool_to_package[tool]]}" for tool in tools])
    print(TextColours.WARNING + "Please install them with:\nSlower -->  conda install -c bioconda " + install_commands + TextColours.ENDC)
    print(TextColours.WARNING + "Note that bcftools may have to be installed with: conda install -c conda-forge bcftools=1.20\n" + TextColours.ENDC)
...
def one_sample(file_to_process):
    try:
        ...
        print(TextColours.HEADER + f"Processing sample: {sample_name} ({lines-1}/{len(fastq_to_process)})" + TextColours.ENDC)
        sample_folder=os.path.join(output_path,sample_name)
        if not os.path.exists(sample_folder):
            os.makedirs(sample_folder, exist_ok=True)
        with open(os.path.join(sample_folder, f"{sample_name}_timings.csv"), "a") as f:
            ...
            elapsed_time = run_cmd([os.path.join(soft_path,'third_party_software','smalt-0.7.6'),'index -k 13 -s 6',ref_copy,concatenated_file_path])
            writer.writerow([sample_name,"smalt-0.7.6 index", elapsed_time])
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','smalt-0.7.6'),f'map -n {ncores_per_sample} -d -1 -y 0.7 -x -f samsoft -o',os.path.join(sample_folder,sample_name+".sam"),ref_copy, fastq_trimmed])
            writer.writerow([sample_name,"smalt-0.7.6 map", elapsed_time])
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','samtools_1.20'),'view -Sh -F 4',os.path.join(sample_folder,sample_name+".sam"),'>',os.path.join(sample_folder,sample_name+".F4.sam")])
            ...
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','samtools_1.20'),'view -Shu',os.path.join(sample_folder,sample_name+'.60.sam'),'| samtools sort -o',os.path.join(sample_folder,sample_name+'.sorted.bam')]) 
            writer.writerow([sample_name,"samtools view and sort", elapsed_time])
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','samtools_1.20'),'index',os.path.join(sample_folder,sample_name+'.sorted.bam')])
            writer.writerow([sample_name,"samtools index", elapsed_time])
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','samtools_1.20'),'faidx',concatenated_file_path])
            writer.writerow([sample_name,"samtools faidx", elapsed_time])
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','bcftools_1.20'),'mpileup','-q -1 -Ou --max-depth 1000000 --fasta-ref',concatenated_file_path,os.path.join(sample_folder,sample_name+'.sorted.bam'),'>',os.path.join(sample_folder,sample_name+'.mpileup.bcf')])    
            writer.writerow([sample_name,"samtools mpileup", elapsed_time])
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','bcftools_1.20'),'call -c --ploidy 1',os.path.join(sample_folder,sample_name+'.mpileup.bcf'),'>',os.path.join(sample_folder,sample_name+'.mpileup.vcf')])
            writer.writerow([sample_name,"bcftools view", elapsed_time])
            ...
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','bcftools_1.20'),'call -vc',os.path.join(sample_folder,sample_name+'.mpileup.bcf'),'>',os.path.join(sample_folder,sample_name+'.snp')])
            writer.writerow([sample_name,"bcftools view 2", elapsed_time])
            ...
            elapsed_time =run_cmd(['python',os.path.join(soft_path,'calculate_genes_presence.py'),sample_folder,concatenated_file_path,mlst_fasta,str(th_qual),str(th_prop),str(th_min)])
    ...
    #seqfinder_file_name=os.path.join(sample_folder,seqfinder_file_name)
            if os.path.isfile(abricate_file_name) and os.path.isfile(seqfinder_file_name):
                ...
                try:
                    run_cmd(['python',os.path.join(soft_path,'abricate_combine_with_seqfinder.py'),abricate_file_name,seqfinder_file_name,default_output_file,efsa_dict,first_line,abricate_version,mlst_fasta,deleted_sequences])
                except:
...
if sample_list_file=="":
    ...
    if not os.path.isfile(sample_list_file):
        ...
        with open(sample_list_file,'r') as f:
                    content=f.read()
                    if "~/" in content:
                        print(TextColours.FAIL +"\nYour sample list file contains a tilde (~). Please provide the full path and run again."+TextColours.ENDC)
                        sys.exit()
        samples=read_csv(sample_list_file)
        for line in samples:
            if "~" in line:
                print("\nYour filepaths contain a tilde (~). Please provide the full path and run again.")
                sys.exit()
