...
import subprocess
import shutil
#TODO: error case where gzip file is not a gzip currently causes CTD
import gzip
from errno import ENOTDIR

userID = subprocess.check_output('whoami').strip().decode('utf-8')
...
if sys.version_info < (3,7):
    sys.exit(TextColours.WARNING +"Python 3.7 or greater is required.\nRecommend creating a new python environment >=3.7 e.g.: conda create -n seqfinder -c conda-forge python=3.10.14 numpy pandas tqdm biopython"+ TextColours.ENDC)

print(TextColours.HEADER + "Checking if required python modules are installed\n" + TextColours.ENDC)
...
if not_installed:
    ...
    print(TextColours.WARNING + "\nStrongly suggest that you create a new environment for APHA SeqFinder: mamba (or conda) create -n seqfinder python=3.10.14\n" + TextColours.ENDC)
    print(TextColours.WARNING + "Then to install the required packages:\nmamba activate seqfinder\nSlower --> conda install -c conda-forge", "numpy pandas tqdm biopython"+ TextColours.ENDC)
    print(TextColours.WARNING + "***OR***\nQuicker --> mamba install -c conda-forge", "numpy pandas tqdm biopython"+ TextColours.ENDC)
    sys.exit()
...
if major is not None and major < 2:
    ...
    print(TextColours.WARNING+"You may have to create a new python environment >=3.7 e.g.: conda create -n seqfinder python=3.10.14"+TextColours.ENDC)
    sys.exit(1)
else: 
    print(TextColours.OKBLUE +"\nPandas version check passed. Proceeding with the pipeline.\n"+TextColours.ENDC)

def is_tool_installed(name):
    """Check whether 'name' is in PATH and marked as executable."""
    try:
        # Add a special case for 'makeblastdb'
        if name == 'makeblastdb':
            subprocess.run([name, '-version'], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        elif name == 'seqtk':
            subprocess.run([name], stderr=subprocess.PIPE,stdout=subprocess.PIPE,)
        else:
            subprocess.run([name, '--version'], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return True
...
def get_tool_version(name):
    ...
    try:
        if name == 'makeblastdb':
            ...
            result = subprocess.run([name], stderr=subprocess.PIPE)
        else:
            result = subprocess.run([name, '--version'], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        try:
            ...
            output = (stdout + stderr).decode('latin1').strip()
        ...
        if name == 'abricate':
            ...
            version = output.split()[1]
        elif name == 'samtools' or name == 'bcftools':
            ...
            # Normalise to just the numeric part (e.g. 1.4 from 1.4-r122)
            if version:
                numeric_version_match = re.match(r'[\d.]+', version)
                version = numeric_version_match.group() if numeric_version_match else version
        else:
            version = None
...
#TODO: why are these tools required while others are in third pary software?
print(TextColours.HEADER + "Checking accessory programs are installed\n" + TextColours.ENDC)
tools = ['makeblastdb', 'mlst', 'abricate', 'samtools', 'bcftools', 'platon', 'mash','seqtk']
required_versions = {'mlst': '2.23.0', 'abricate': '1.0.1', 'makeblastdb': '2.15.0+', 'samtools': '1.21', 'bcftools': '1.21', 'platon': '1.7', 'mash': '2.3','seqtk':'1.4'}
required_versions_install = {'mlst': '2.23.0', 'abricate': '1.0.1', 'blast': '2.15.0', 'samtools': '1.21', 'bcftools': '1.21', 'platon': '1.7', 'mash': '2.3','seqtk':'1.4'}
tool_to_package = {'makeblastdb': 'blast', 'mlst': 'mlst', 'abricate': 'abricate', 'samtools': 'samtools', 'bcftools': 'bcftools', 'platon': 'platon', 'mash': 'mash','seqtk': 'seqtk'}
...
for tool in tools:
    if not is_tool_installed(tool):
        ...
        elif tool_version != required_versions[tool]:
            print(TextColours.WARNING + f"{tool} version mismatch: found {tool_version}, expected {required_versions[tool]}" + TextColours.ENDC)
            incorrect_version_tools.append(tool)
        else:
            print(TextColours.OKBLUE + f"{tool} version {tool_version} is correct." + TextColours.ENDC)

if not_installed_tools or incorrect_version_tools:
    problematic_tools = not_installed_tools + incorrect_version_tools
    print(TextColours.FAIL + f"The following tools are not installed or the incorrect version is installed: {', '.join(problematic_tools)}" + TextColours.ENDC)
    install_commands = ' '.join([f"{tool_to_package[tool]}={required_versions_install[tool_to_package[tool]]}" for tool in problematic_tools])
    print(TextColours.WARNING + "\nIt is recommended that you create a new environment for SeqFinder if you do not already have one.\n\nThis is how you can create a new environment: conda create -n seqfinder python=3.10.14\n" + TextColours.ENDC)
    print(TextColours.WARNING + "Please install the required tools with:\nSlower -->  conda install -c bioconda " + install_commands + TextColours.ENDC)
    print(TextColours.WARNING + "**Note** that bcftools may have to be installed with: conda install -c conda-forge bcftools=1.21\n" + TextColours.ENDC)
    print(TextColours.WARNING + "***OR***\n\nQuicker --> mamba install -c bioconda " + install_commands + TextColours.ENDC)
    sys.exit()



# If all required packages are installed, proceed with the rest of the script
...
def find_shortest_reference_sequence(reference):
    ...
    for record in SeqIO.parse(reference, "fasta"):
        ...
        # Update shortest sequence information if the current sequence is shorter
...
def calculate_average_read_length(input_file, num_reads=1000):
    # Check if the file exists before opening if it exists then we count the first 1000 reads to get an average read length
...
def split_fastq(fastx_location, input_file, output_file_1, output_file_2):
    ...
    # Using the existing function for calculate_average_read_length
    average_length = calculate_average_read_length(input_file)
...
def process_files_read_length(files_to_process, reference, fastx_location):
    ...
    print(TextColours.HEADER + f"\nChecking if average read-length for any isolate is 75% greater than the shortest reference sequence: {shortest_reference_name} -> {shortest_reference_length}bp\n" + TextColours.ENDC)

    # Identify isolates with long reads
    ...
    if num_positive_indexes > 0:
        ...
        os.makedirs(fastx_location, exist_ok=True)

        # Prepare list of output file names
        concatenated_output_files = []
        for index in df[larger_than_75_percent].index:
            sublist = shared_fastq_to_process[index]
            input_file_forward = sublist[0]
        ...
        # Process splitting
        with Pool() as pool:
            positive_indexes_data_list = pool.starmap(
                process_positive_index,
                [(index, shared_fastq_to_process, concatenated_output_file)
                 for index, concatenated_output_file in zip(df[larger_than_75_percent].index, concatenated_output_files)]
            )

        # Flatten results
        #HACK: doesn't seem to do anything
        #positive_indexes_data = [item for sublist in positive_indexes_data_list for item in sublist]

        # Recalculate average read length for updated forward reads and update list
        for index in df[larger_than_75_percent].index:
            sublist = shared_fastq_to_process[index]
            forward_file = sublist[0]  # This is now the new split file
            avg_forward = calculate_average_read_length(forward_file)
            if avg_forward < 80:
                new_avg = int(avg_forward * 0.85)
                sublist[4] = new_avg
                shared_fastq_to_process[index] = sublist
    else:
    ...
    # Convert shared dict back to list
...
def trimming(sample_name,sample_folder,r1_file,r2_file,min_read_length):
    ...
    try:
        run_cmd([os.path.join(soft_path,'third_party_software','fastp'),' -i '+r1_file+' -I '+r2_file+' -o '+r1_file_trimmed_paired+' -O '+r2_file_trimmed_paired+' --unpaired1 '+unpaired_trimmed+' --unpaired2 '+unpaired_trimmed+' -l '+str(min_read_length)+' -r --cut_right_window_size 10 -w '+str(ncores_per_sample)+' -j /dev/null -h /dev/null'])
    except:
        print("fastp error, exiting.")
        sys.exit()
    fastq_trimmed=os.path.join(sample_folder,sample_name+'.trimmed.fastq.gz')
    if not os.path.isfile(r1_file_trimmed_paired):
        print(TextColours.FAIL +'\nfastp output cannot be accessed. Please check your rawreads.\n.'+ TextColours.ENDC)
        sys.exit()
...
def combine_tables_of_results(mother_path, pattern, out_file):
    ...
    try:
        ...
        print(TextColours.FAIL+f"An index error occurred, likely because there are no files to combine: {z}"+TextColours.ENDC)
...
def get_mlst(fastq_to_process):
    try:
        ...
        run_cmd_silent(["mlst","--quiet", "--threads",str(ncores_per_sample),"--csv", fasta_file, ">>", mlst_output])
        return True
    except Exception as e:
        print(TextColours.FAIL+"Something went wrong with the mlst stage:", str(e)+TextColours.FAIL)
        return False
...
def failures(removed_isolates):
    try:
        ...
        print(TextColours.FAIL+"Something went wrong when dealing with failures:", str(e)+TextColours.FAIL)
...
def build_reference(fastq_to_process):
    try:
        ...
        if not os.path.exists(abricate_folder):
            os.makedirs(abricate_folder, exist_ok=True)
        ...
        files_to_check = ["sequences", "sequences.ndb", "sequences.nhd", "sequences.nhi", "sequences.nhr", "sequences.nin", "sequences.njs", "sequences.nog", "sequences.not", "sequences.nsq", "sequences.ntf", "sequences.nto"]
        for file in files_to_check:
            if not os.path.exists(os.path.join(abricate_folder, file)):
                print(TextColours.FAIL+f"Error: {file} has not been created in the abricate folder."+TextColours.ENDC)
                sys.exit()
        return True
    except Exception as e:
        print(TextColours.FAIL+"Something went wrong with the reference building stage:", str(e)+TextColours.ENDC)
        return False
...
def fasta_to_fastq(fasta_file, fastq_file):
    with open(fasta_file, 'r') as f_in, open(fastq_file, 'w') as f_out:
        ...
        for line in f_in:
            if line.startswith('>'):
                if sequence and not all(base == 'n' for base in sequence):
                    # Write the previous header, sequence, and quality scores
                    quality_scores = ''.join('~' if base in 'ATGC' else '!' for base in sequence)
                    f_out.write(f"{header}\n{sequence}\n+\n{quality_scores}\n")
                # Update the header
                header = f"@{line[1:].strip()}"
                sequence = ""
            else:
                # Append to the sequence
                sequence += line.strip()
        # Write the last header, sequence, and quality scores if it's not all 'n'
        if sequence and not all(base == 'n' for base in sequence):
            quality_scores = ''.join('~' if base in 'ATGC' else '!' for base in sequence)
            f_out.write(f"{header}\n{sequence}\n+\n{quality_scores}\n")

def mash_screen(trimmed_fastq, reference_msh, output_file):
    with open(output_file, "w") as out:
        subprocess.run(["mash", "screen","-p", str(ncores_per_sample), reference_msh, trimmed_fastq], stdout=out)

def parse_mash_output(mash_output_path, isolate_name):
    rows = []
    with open(mash_output_path, "r") as infile:
        for line in infile:
            parts = line.strip().split('\t')
            if len(parts) >= 6:
                identity, shared_hashes, median_multiplicity, p_value, query_id, query_comment = parts[:6]
                try:
                    mapped, total = map(int, shared_hashes.split('/'))
                    percentage_mapped = (mapped / total) * 100
                except:
                ...
                rows.append({
                    ...
                    "median-multiplicity": median_multiplicity,
                    "p-value": p_value,
                    "query-ID": query_id,
                    "query-comment": query_comment.strip(),
                    "Percentage_mapped": percentage_mapped
                })
    return pd.DataFrame(rows)


def filter_and_extract_per_isolate(fastq_trimmed, reference_msh, fasta_path, output_folder):
    try:
        # Extract isolate name from file path
        
        isolate = os.path.splitext(os.path.basename(fastq_trimmed))[0]
        isolate_name = isolate.split(".trimmed.fastq")[0]
        mash_output = os.path.join(output_folder, f"{isolate}_mashout.tab")
        print(TextColours.INFO+f"\nRunning mash screen on {isolate_name}\n"+TextColours.ENDC)
        # Run mash screen
        mash_screen(fastq_trimmed, reference_msh, mash_output)

        # Parse mash output
        df = parse_mash_output(mash_output, isolate)

        # Apply two-tiered filtering
        mask_general = (~df["query-comment"].str.contains("PlasR|Integ", case=False)) & (df["Percentage_mapped"] > 10)
        mask_special = (df["query-comment"].str.contains("PlasR|Integ", case=False)) & (df["Percentage_mapped"] > 1)
        filtered_df = df[mask_general | mask_special]

        # Extract matching contigs
        contigs_to_keep = set(filtered_df["query-comment"])
        selected_contigs = [
            record for record in SeqIO.parse(fasta_path, "fasta")
            if record.id in contigs_to_keep
        ]

        # Write filtered FASTA
        os.makedirs(output_folder, exist_ok=True)
        output_fasta = os.path.join(output_folder, f"{isolate_name}_filtered_reference.fasta")
        SeqIO.write(selected_contigs, output_fasta, "fasta")

        print(TextColours.INFO+f"\n{isolate_name} hit {len(selected_contigs)} genes using mash that will now be mapped against with smalt\n"+TextColours.ENDC)

    except Exception as e:
        print(TextColours.FAIL+f"Error processing {fastq_trimmed}: {e}"+TextColours.ENDC)



def count_reads(fastq_file):
    with gzip.open(fastq_file, 'rt') as f:
        return sum(1 for _ in f) // 4  # 4 lines per read
...
def one_sample(file_to_process):
    try:
        ...
        min_read_length=file_to_process[4]
        mlst_file_path = file_to_process[5]
        ref_copy_file_path = ref_copy

        #HACK: write pids to different files
        current_pid = os.getpid()  # Get the current process's PID
        
        # Write current sample's PID to its own processed.csv
        with open(os.path.join(sample_folder, "processed.csv"), "a", newline='') as f:
            writer = csv.writer(f)
            writer.writerow([sample_name, current_pid])
        # Count how many samples have been processed by checking for processed.csv files
        processed_files = [d for d in os.listdir(output_path)
        if os.path.isfile(os.path.join(output_path, d, "processed.csv"))        ]
        processed_count = len(processed_files)
        
        print(TextColours.HEADER+f"Processing sample: {sample_name} ({processed_count}/{len(fastq_to_process)})" +TextColours.ENDC)
            
        #TODO: I am suspcious of this, it seems long
        with open(os.path.join(sample_folder, f"{sample_name}_timings.csv"), "a") as f:
            writer = csv.writer(f)
            writer.writerow(["Isolate","Process", "Time"])
            start_time_fastp= time.time()
            print(TextColours.INFO+f"\nRunning fastp on {sample_name}\n"+TextColours.ENDC)
            fastq_trimmed=trimming(sample_name,sample_folder,r1_file,r2_file,min_read_length)
            end_time_fastp= time.time()
            elapsed_time_fastp = end_time_fastp-start_time_fastp
            writer.writerow([sample_name,"fastp", elapsed_time_fastp])
            filter_and_extract_per_isolate(fastq_trimmed,reference_msh,ref_copy_file_path,reference_folder)
            filtered_reference_filepath = os.path.join(reference_folder, f"{sample_name}_filtered_reference.fasta")
            #HACK: cdhit element
            if cdhit:
                #has to be v4.7, but as bad as seqkt for versioning
                #overide mash filtered fasta with cdhit
                run_cmd(['python',os.path.join(soft_path,'cdhit_reader.py'),filtered_reference_filepath,filtered_reference_filepath+"_chit80_clustered.fasta"])
                filtered_reference_filepath = filtered_reference_filepath+"_chit80_clustered.fasta"
            if mlst_file_path != "None_found":
                # Read contents of mlst_fasta
                with open(mlst_file_path, 'r') as mlst_file:
                    mlst_contents = mlst_file.read()
                # Read contents of ref_copy
                with open(filtered_reference_filepath, 'r') as filtered_reference_file:
                    filtered_reference_contents = filtered_reference_file.read()       
                # Concatenate contents
                concatenated_contents = mlst_contents +'\n'+ filtered_reference_contents  
                # Output path for the concatenated file
                concatenated_file_path = filtered_reference_filepath  
              ...
              concatenated_file_path=filtered_reference_filepath
            num_reads = count_reads(fastq_trimmed)
            if num_reads > 4000000:
                print(TextColours.INFO+f"\nSubsampling {sample_name} to 4 million reads\n"+TextColours.ENDC)
                subsampled_file = fastq_trimmed.split(".trimmed.fastq.gz")[0] + "_subsampled.fastq.gz"
                run_cmd(['seqtk sample -s22 -2 ',fastq_trimmed,' 4000000 | gzip > ',subsampled_file])
                fastq_trimmed=subsampled_file
            else:
                fastq_trimmed=fastq_trimmed
            elapsed_time = run_cmd([os.path.join(soft_path,'third_party_software','smalt-0.7.6'),'index -k 13 -s 3',concatenated_file_path,concatenated_file_path])
            writer.writerow([sample_name,"smalt-0.7.6 index", elapsed_time])
            print(TextColours.INFO+f"\nRunning smalt on {sample_name}\n"+TextColours.ENDC)
            elapsed_time =run_cmd([os.path.join(soft_path,'third_party_software','smalt-0.7.6'),f'map -n {ncores_per_sample} -d -1 -y 0.7 -x -f samsoft -o',os.path.join(sample_folder,sample_name+".sam"),concatenated_file_path, fastq_trimmed])
            writer.writerow([sample_name,"smalt-0.7.6 map", elapsed_time])
            if not os.path.isfile(os.path.join(sample_folder,sample_name+".sam")):
                print(TextColours.FAIL +'\nsmalt failed.\n.'+ TextColours.ENDC)
                raise ValueError("Exiting")
            print(TextColours.INFO+f"\nRunning samtools on {sample_name}\n"+TextColours.ENDC)
            elapsed_time =run_cmd(['samtools',f'view -@ {ncores_per_sample} -Sh -F 4',os.path.join(sample_folder,sample_name+".sam"),'>',os.path.join(sample_folder,sample_name+".F4.sam")])
            writer.writerow([sample_name,"samtools view", elapsed_time])
            start_time_sam_flag_change=time.time()
            sam_flag_changed(os.path.join(sample_folder,sample_name+".F4.sam")) #produces .60.sam
            end_time_sam_flag_change= time.time()
            elapsed_time_sam_flag_change = end_time_sam_flag_change-start_time_sam_flag_change
            writer.writerow([sample_name,"sam flag change", elapsed_time_sam_flag_change])
            elapsed_time =run_cmd(['samtools',f'view -@ {ncores_per_sample} -Shu',os.path.join(sample_folder,sample_name+'.60.sam'),f'| samtools sort -@ {ncores_per_sample} -o',os.path.join(sample_folder,sample_name+'.sorted.bam')]) 
            writer.writerow([sample_name,"samtools view and sort", elapsed_time])
            elapsed_time =run_cmd(['samtools','index',os.path.join(sample_folder,sample_name+'.sorted.bam')])
            writer.writerow([sample_name,"samtools index", elapsed_time])
            elapsed_time =run_cmd(['samtools','faidx',concatenated_file_path])
            writer.writerow([sample_name,"samtools faidx", elapsed_time])
            print(TextColours.INFO+f"\nRunning bcftools on {sample_name}\n"+TextColours.ENDC)
            elapsed_time =run_cmd(['bcftools','mpileup',f'--threads {ncores_per_sample} -q -1 -Ou --max-depth 1000000 --fasta-ref',concatenated_file_path,os.path.join(sample_folder,sample_name+'.sorted.bam'),'>',os.path.join(sample_folder,sample_name+'.mpileup.bcf')])    
            writer.writerow([sample_name,"bcftools mpileup", elapsed_time])
            elapsed_time =run_cmd(['bcftools',f'call --threads {ncores_per_sample} -Oz -c --ploidy 1',os.path.join(sample_folder,sample_name+'.mpileup.bcf'),'>',os.path.join(sample_folder,sample_name+'.mpileup.vcf.gz')])
            writer.writerow([sample_name,"bcftools call", elapsed_time])
            elapsed_time =run_cmd(['bcftools',f'call --threads {ncores_per_sample} -c --ploidy 1',os.path.join(sample_folder,sample_name+'.mpileup.bcf'),'>',os.path.join(sample_folder,sample_name+'.mpileup.vcf')])
            writer.writerow([sample_name,"bcftools call no zip", elapsed_time])
            elapsed_time=run_cmd(['bcftools','index',os.path.join(sample_folder,sample_name+'.mpileup.vcf.gz')])
            writer.writerow([sample_name,"bcftools index", elapsed_time])
            elapsed_time=run_cmd(['bcftools','consensus -a n -s - -e \'TYPE="indel"\' -f',concatenated_file_path,os.path.join(sample_folder,sample_name+'.mpileup.vcf.gz'),'>',os.path.join(sample_folder,sample_name+'_consensus.fasta')])
            fasta_file_bcftools= os.path.join(sample_folder,sample_name+'_consensus.fasta')
            fastq_file=os.path.join(sample_folder,sample_name+'.fq')
            fasta_to_fastq(fasta_file_bcftools, fastq_file)
            writer.writerow([sample_name,"bcftools consensus", elapsed_time])
            #elapsed_time =run_cmd(['perl',os.path.join(soft_path,'third_party_software','vcfutils.pl'),'vcf2fq',os.path.join(sample_folder,sample_name+'.mpileup.vcf'),'>',os.path.join(sample_folder,sample_name+'.fq')])   
            #writer.writerow([sample_name,"vcfutils", elapsed_time])
            elapsed_time =run_cmd(['bcftools',f'call --threads {ncores_per_sample} --ploidy 1 -vc',os.path.join(sample_folder,sample_name+'.mpileup.bcf'),'>',os.path.join(sample_folder,sample_name+'.snp')])
            writer.writerow([sample_name,"bcftools call 2", elapsed_time])
            start_time_snps_filter= time.time()
            ...
            end_time_snps_filter= time.time()
            elapsed_time_snps_filter = end_time_snps_filter-start_time_snps_filter
            writer.writerow([sample_name,"snps_filter", elapsed_time_snps_filter])
            ...
            print(TextColours.INFO+f"\nRunning calculate_genes_presence.py on {sample_name}\n"+TextColours.ENDC)
            elapsed_time =run_cmd(['python',os.path.join(soft_path,'calculate_genes_presence.py'),sample_folder,concatenated_file_path,mlst_file_path,str(th_qual),str(th_prop),str(th_min),ref_copy])
            writer.writerow([sample_name,"calculate_genes_presence", elapsed_time])
            f.flush()
        ...
        for filename in os.listdir(sample_folder):
            if "CompareTo" in filename:
                ...
                try:
                    #TODO: remove database_type dependency from good_snps_filtering?
                    print(TextColours.INFO+f"\nRunning good_snps_filtering.py on {sample_name}\n"+TextColours.ENDC)
                    run_cmd(['python', os.path.join(soft_path, 'good_snps_filtering.py'), os.path.join(sample_folder, compare_file), str(percentageID), str(numofsnps), efsa_dict, database_type, vir_dict, reference_name, bio_metal_dict,snp_pattern_dict])
        ...
        print(TextColours.FAIL + "Therefore, "+sample_name +" wasn't processed." + TextColours.ENDC)
        ...
        if os.path.isfile(mlst_error):
            ...
            failed_isolate_df.to_csv(output_file_good_snps, index=False)
            failed_isolate_df.to_csv(output_file_abricate_seqfinder, index=False)
            #HACK: don't see why we're doing this, since we ain't for the others
    ...
    default_output_file=os.path.join(sample_folder,compare_file)
    deleted_sequences=os.path.join(sample_folder,sample_name+"_DeletedSequences.csv")
    
    with open(seqfinder_file_name,"r") as f:
        contents = f.read()
        #TODO: decide if we should run abricate anyway
        if "No genes passed filter" in contents:
            #HACK Keep .fq file rather than removing, using same logic as normal run
            ...
            delete_files(sample_folder,'.vcf.gz')
            delete_files(sample_folder,'.vcf.gz.csi')
            delete_files(sample_folder,'_consensus.fasta')
            ...
            #HACK: stop removing the ref folder
            #run_cmd(["rm -r "+reference_folder])
            ...
            #HACK: cdhit abricate option
            if cdhit:
                #make a new abricate db
                cd_abricate_folder = filtered_reference_filepath.replace(".fasta","")
                os.makedirs(cd_abricate_folder)
                run_cmd_silent(['cp',filtered_reference_filepath,os.path.join(cd_abricate_folder,"sequences")])
                cd_abricate_db = cd_abricate_folder.split("/")[-1]
                run_subprocess_silent(['makeblastdb','-in',os.path.join(cd_abricate_folder,"sequences"),'-dbtype','nucl','-hash_index'])        
                os.system("abricate --datadir {} --csv --mincov 10 --minid 75 --db {} {} > {}".format(reference_folder, cd_abricate_db, fasta_file, abricate_file_name))
            else:
                print(TextColours.INFO+f"\nRunning abricate on {sample_name}\n"+TextColours.ENDC)
                os.system("abricate --datadir {} --csv --mincov 10 --minid 75 --db {} {} > {}".format(reference_folder, reference_name, fasta_file, abricate_file_name))
            ...
            abricate_file=pd.read_csv(abricate_file_name)

            rows_to_drop=["RESISTANCE","ACCESSION","PRODUCT"]
            abricate_file=abricate_file.drop(rows_to_drop,axis=1)
            abricate_file["Strain"]=abricate_file["#FILE"].str.split("/").str[-1].str.split(".").str[0]
            abricate_file["Strain"]=abricate_file["Strain"].str.replace("_spades","")
            abricate_file["Strain"]=abricate_file["Strain"].str.replace("_UC","")
            abricate_file.insert(1, "Strain", abricate_file.pop("Strain"))
            abricate_file["Chromosome"]=abricate_file["GENE"].apply(lambda x: 'Yes' if 'Chr' in x else 'No')
            abricate_file.insert(7, "Chromosome", abricate_file.pop("Chromosome"))
            abricate_file["Stripped_gene_name"] = abricate_file["GENE"].apply(lambda x: x.split("_", 1)[1] if "_" in x else x)
            abricate_file["Stripped_gene_name"] = abricate_file["Stripped_gene_name"].str.replace("_Chr","")
            abricate_file.insert(8, "Stripped_gene_name", abricate_file.pop("Stripped_gene_name"))
            abricate_file["Generated_by"]=userID
            abricate_file_filter=abricate_file["SEQUENCE"][(abricate_file["Chromosome"]=="No")&(~abricate_file["GENE"].str.contains("PlasR"))&(~abricate_file["GENE"].str.contains("Integ"))].drop_duplicates()
            abricate_file.to_csv(abricate_file_name,index=False)
            
            #TODO: suggest we split platon out and apply as fully separete function, possibly afterwards and to whole fasta?
            if database_type=="AMR":
                ##Setting up to run platon to determine the contigs that could be plasmids
                headers_to_extract = abricate_file_filter.tolist()
                # Create a set for faster lookup
                headers_set = set(headers_to_extract)
                amr_only_fasta=fasta_file.split(os.sep)[-1].split(".")[0] + "_AMR.fasta"
                amr_only_fasta=os.path.join(sample_folder,amr_only_fasta)
                # Open the output file for writing
                with open(amr_only_fasta, "w") as amr_only_fasta_output:
                    # Iterate through the records in the input FASTA file
                    for record in SeqIO.parse(fasta_file, "fasta"):
                        # Check if the record's ID is in the list of headers to extract
                        if record.id in headers_set:
                            # Write the record to the output file
                            SeqIO.write(record, amr_only_fasta_output, "fasta")
                platon_output=os.path.join(sample_folder,sample_name+"_platon")
                #TODO: platon is apparently disliked by users...
                #HACK: platon toggle
                if platon:
                    try:
                        print(TextColours.INFO+f"\nRunning platon on {sample_name}\n"+TextColours.ENDC)
                        run_cmd(['platon',f'--threads {ncores_per_sample} --db {platon_db_path} {amr_only_fasta} --output {platon_output}']) 
                    except:
                        print(TextColours.FAIL +'\n Issue with running platon. Exiting\n'+TextColours.ENDC)
                        raise ValueError('exiting')
                platon_file = os.path.join(sample_folder, platon_output, fasta_file.split(os.sep)[-1].split(".")[0] + "_AMR.tsv")
                headers = ["ID","Length","Coverage","# ORFs","RDS","Circular","Inc Type(s)","# Replication","# Mobilization","# OriT","# Conjugation	","# AMRs","# rRNAs","# Plasmid Hits"]
                if not os.path.exists(platon_file):
                    os.makedirs(os.path.dirname(platon_file), exist_ok=True)  # Ensure directory exists
                    with open(platon_file, mode='w', newline='', encoding='utf-8') as file:
        ...
        #seqfinder_file_name=os.path.join(sample_folder,seqfinder_file_name)
                if os.path.isfile(abricate_file_name) and os.path.isfile(seqfinder_file_name):
                    ...
                    try:
                        run_cmd(['python',os.path.join(soft_path,'abricate_combine_with_seqfinder.py'),abricate_file_name,seqfinder_file_name,default_output_file,efsa_dict,first_line,abricate_version,mlst_file_path,deleted_sequences,platon_file,snp_pattern_dict,gene_dict])
                    except:
                        print(TextColours.FAIL +'\nabricate_combine_with_seqfinder.py failed. Exiting\n'+TextColours.ENDC)
                        raise ValueError('exiting')
            elif database_type !="AMR":
                columns = ['ID','Length','Coverage','# ORFs','RDS','Circular','Inc Type(s)','# Replication','# Mobilization','# OriT','# Conjugation','# AMRs','# rRNAs','# Plasmid Hits']
                platon_df = pd.DataFrame(columns=dict.fromkeys(columns, []))
                platon_file_filename = os.path.join(sample_folder,sample_name+"_platon.tsv")
                platon_df.to_csv(platon_file_filename, sep='\t', index=False)
        ...
        #seqfinder_file_name=os.path.join(sample_folder,seqfinder_file_name)
                if os.path.isfile(abricate_file_name) and os.path.isfile(seqfinder_file_name):
                    ####### combination seqfinder abricate
                    try:
                        print(TextColours.INFO+f"\nRunning abricate_combine_with_seqfinder.py on {sample_name}\n"+TextColours.ENDC)
                        run_cmd(['python',os.path.join(soft_path,'abricate_combine_with_seqfinder.py'),abricate_file_name,seqfinder_file_name,default_output_file,efsa_dict,first_line,abricate_version,mlst_file_path,deleted_sequences,platon_file_filename,snp_pattern_dict,gene_dict])
                    except:
                        print('\nabricate_combine_with_seqfinder.py failed. Exiting\n')
                        raise ValueError('exiting')
                    
    ########## deleting unwanted files
    #TODO: I think we should move this to a separate fuction that gets called after every exit
    delete_files(sample_folder,'.fastq.gz')
    delete_files(sample_folder,'.sam')
    delete_files(sample_folder,'.bam')
    delete_files(sample_folder,'.bai')
    ...
    #run_cmd(["rm -r "+reference_folder])
    if database_type=="AMR":
        run_cmd(["rm -r "+platon_output])
    #Turn off if debugging
    #delete_files(sample_folder, '_timings.csv')          
...
percentageID=70
#TODO: adjust this to actually make sense instead of same limit for all gene lengths
numofsnps=100

###########Command line arguments parsing
soft_path=""
reference=""
#TODO: get rid of database_type and make generic?
database_type=""
data_path=""
R1_pattern=""
...
mlst_picker=""
results_path=""
#TODO: get rid of efsa_dict, vir_dict, bio_metal_dict as seperate args and make generic, e.g. annotation_dict?
efsa_dict=""
vir_dict=""
bio_metal_dict=""
fastas_folder=""
...
platon_db_path=""
snp_pattern_dict=""
#TODO: derive the path from the reference database and generate on the fly if can't find?
AMR_reference_msh=""
Virulence_reference_msh=""
Biocide_Metal_reference_msh=""
reference_msh=""
gene_dict=""
#HACK: to allow injection of bool from args file that switches off user inputs for easy script calling
pipeline=False
#HACK: to allow injection of bool from args to switch on cdhit clustering and db rename
#TODO: I want to just apply this to the core db at the beginning of the run and the end, but it could lower mash screen sensitivity...
#TODO: apply the best match finder bit to restore original db names near the end - before snp_pattern_dict isn't possible for now!
cdhit=False
#HACK: to allow platon off switch
platon=True
...
print(TextColours.HEADER +'\nChecking the arugments file for errors'+TextColours.ENDC)
args=sys.argv
if len(args)>1:
    arguments_file=args[1]
    if not os.path.exists(arguments_file):    
        print(TextColours.FAIL +"Argument file not given or doesn't exist. Please re run with /full/path/to/arguments/file/arguments_file.args"+TextColours.ENDC)
        sys.exit()
else:
    print(TextColours.FAIL +"Please provide an arguments file. Check readme.txt for more info"+ TextColours.ENDC)
    sys.exit()
...
with open(arguments_file,'r') as f:
    content=f.read()
    if "~/" in content:
        print(TextColours.FAIL +"\nYour argument file contains a tilde (~). Please provide the full path and run again."+TextColours.ENDC)
        sys.exit()
...
#HACK: option to switch this off for pipelines
if not pipeline:
    #TODO: we could insert a series of y/n questions for things like platon?
    ### Checking if user has downloaded SeqFinder or using bactipipes version
    soft_path_checker="Bactipipes/BactiPipes_SCE3/AMR_github"
    
    if soft_path_checker in soft_path:
        print(TextColours.OKBLUE+"\nUsing SeqFinder version directly from Bactipipes, well done. Continuing...\n"+TextColours.ENDC)
    else:
        # Prompt the user if they want to continue
        user_input = input(TextColours.WARNING+"\nIt seems you are not running SeqFinder from Bactipipes, this is not recommended. Do you want to continue? (y/n): "+TextColours.ENDC)
        if user_input =='y':
            print(TextColours.WARNING+"\nContinuing, but don't say I didn't warn you...\n"+TextColours.ENDC)
        elif user_input !='y':
            print(TextColours.FAIL+"\nExiting..."+TextColours.ENDC)
            sys.exit()

### Check to make sure scripts used by this script are actually callable
if not os.path.isfile(os.path.join(soft_path, 'calculate_genes_presence.py')):
    print(TextColours.FAIL +'\ncalculate_genes_presence.py script cannot be accessed. Please check your soft_path:'+soft_path+ TextColours.ENDC)
    sys.exit()   

if not os.path.isfile(os.path.join(soft_path, 'good_snps_filtering.py')):
    print(TextColours.FAIL +'\ngood_snps_filtering.py script cannot be accessed. Please check your soft_path:'+soft_path+ TextColours.ENDC)
    sys.exit()      

if not os.path.isfile(os.path.join(soft_path, 'abricate_combine_with_seqfinder.py')):
    print(TextColours.FAIL +'\nabricate_combine_with_seqfinder.py script cannot be accessed. Please check your soft_path:'+soft_path+ TextColours.ENDC)
    sys.exit()  
...
########## checking that fastas folder exists as long as sample_list_file is not provided
if sample_list_file == '':
    if not os.path.exists(fastas_folder):
        print(TextColours.FAIL +"fastas_folder doesn't exist "+fastas_folder+TextColours.ENDC)
        sys.exit()

########## checking that paths provided in the template argument exist 
if not os.path.exists(soft_path):
    print(TextColours.FAIL +'\nIncorrect path to SeqFinder provided in the arguments file, make sure this is correct above.'+ TextColours.ENDC)
    sys.exit()
if os.path.isfile(soft_path):
    print(TextColours.FAIL +'\nYou have pointed the soft_path to a file not the seqfinder folder within the arguments file. Please change this and try again.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(reference):
    print(TextColours.FAIL +'\nReference file cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
...
if not os.path.isfile(efsa_dict):
    print(TextColours.FAIL +'\nEFSA dictionary cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(vir_dict):
    print(TextColours.FAIL +'\nVirulence dictionary cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(bio_metal_dict):
    print(TextColours.FAIL +'\nBiocide and metal resistance dictionary cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(mlst_scheme_dictionary):
    print(TextColours.FAIL +'\nMLST scheme dictionary cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(snp_pattern_dict):
    print(TextColours.FAIL +'\nSNP pattern dictionary cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()   
if not os.path.isfile(gene_dict):
    print(TextColours.FAIL +'\ngene dictionary cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(AMR_reference_msh):
    print(TextColours.FAIL +'\nAMR reference mash file cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(Virulence_reference_msh):
    print(TextColours.FAIL +'\nVirulence reference mash file cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()
if not os.path.isfile(Biocide_Metal_reference_msh):
    print(TextColours.FAIL +'\nBiocide and metal reference mash file cannot be located, make sure you have provided the correct path above.'+ TextColours.ENDC)
    sys.exit()

#TODO: shouldn't have to release a new SeqFinder version just for a db update, but this is good for catching accidents...
#HACK: option to switch this off for pipelines
if not pipeline:
    reference_name=reference.split(os.sep)[-1].split(".")[0]
    if database_type == "AMR" and reference_name != "AMR_with_plasmids_20250709":
        print(TextColours.WARNING +"You have selected database type of 'AMR' but not the expected AMR reference. This is not recommended or supported.\n"+ TextColours.ENDC)
        value=input(TextColours.QUESTION+ "Do you wish to continue anyway (y|n)?\n"+TextColours.ENDC)
        if value!='y':
            sys.exit()
    
    if database_type == "BAM" and reference_name != "Biocide_Metals_Resistance_2025-06-19":
        print(TextColours.WARNING +"You have selected database type of 'BAM' but not the expected biocide and metal reference. This is not recommended or supported.\n"+ TextColours.ENDC)
        value=input(TextColours.QUESTION+ "Do you wish to continue anyway (y|n)?\n"+TextColours.ENDC)
        if value!='y':
            sys.exit()
            
    if database_type == "VIR" and reference_name != "VirulenceFactors_2024_12_04":
        print(TextColours.WARNING +"You have selected database type of 'VIR' but not the expected virulence reference. This is not recommended or supported.\n"+ TextColours.ENDC)
        value=input(TextColours.QUESTION+ "Do you wish to continue anyway (y|n)?\n"+TextColours.ENDC)
        if value!='y':
            sys.exit()

#TODO: generate these on the fly if they not exist - doesn't take long
if database_type == "AMR":
    reference_msh=AMR_reference_msh
elif database_type =="VIR":
    reference_msh=Virulence_reference_msh
elif database_type =="BAM":
    reference_msh=Biocide_Metal_reference_msh
else:
    reference_msh=reference_msh       
...
with open(reference) as file:
    for line_number, line in enumerate(file, start=1):
        line = line.rstrip('\n')
        if re.search(r"^>[^a-zA-Z0-9]|[^a-zA-Z0-9>\-_.]|>{2,}|^\s*$", line):
            error_found = True
            print(f"\nLine {line_number}: {line}")
            if re.search(r"^\s*$", line):
                print("This line is empty.\n")
            else:
                print(TextColours.WARNING + "This line contains illegal characters or does not have an alphanumeric character directly following '>'.\n" + TextColours.ENDC)
                print(TextColours.WARNING + "Only '>', '_', '-', and '.' are permitted in headers, and headers must have an alphanumeric character directly following '>'.\n" + TextColours.ENDC)
                print(TextColours.WARNING + "Suggested fix:\n  sed -e '/^>/ s/>[^[:alnum:]]/>_/g' -e '/^>/ s/__*/_/g' your_reference_file > your_fixed_reference_file\n" + TextColours.ENDC)

if error_found:
    print(TextColours.WARNING + "\nOne or more issues were found in your reference file. Please fix them before proceeding.\n" + TextColours.ENDC)
else:
...
if os.path.exists(output_path):
    #HACK: option to switch this off for pipelines - just can the run for pipelines
    if pipeline:
        print(TextColours.WARNING+"\nAn output folder already exists in this location. Aborted"+TextColours.ENDC)
        sys.exit()
    print(TextColours.WARNING+"\nAn output folder already exists in this location. Running SeqFinder like this may result in unexpected behaviour if there are pre-existing results.\n\nPlease consider deleting or changing the output location."+TextColours.ENDC)
    value=input(TextColours.QUESTION+'\nWould you like to proceed anyway (y/n)?\n'+TextColours.ENDC)
    if value!='y':
        sys.exit()
        

print(TextColours.HEADER +"Checking samples to be run\n"+ TextColours.ENDC)
if sample_list_file=="":
    if not os.path.exists(data_path) or not os.path.exists(fastas_folder):
        ...
        summary=[["R1_fastq","R2_status","fasta_status","size","min_trimming_length"]]
        min_trimming_length=80
        ...
        for fastq_R1 in fastq_R1s:
            ...
            if sample_name in sample_names:
                ...
                #HACK: this is the pipe operator's fault; just keep the first one
                else:
                    user_input = 'y'
            ...
            if fastq_R1 == fastq_R2:
                print(TextColours.WARNING+"R1 and R2 are the same file, make sure you have chosen the correct R1_pattern in your template file\n"+TextColours.ENDC)
                sys.exit()
            if os.path.isfile(os.path.join(data_path,fastq_R2)):
                ...
                if os.path.getsize(os.path.join(data_path, fastq_R1)) < 10000000 or os.path.getsize(os.path.join(data_path, fastq_R2)) < 10000000:   
                    remove_small_files=""
                    #HACK: option to switch this off for pipelines, deafulting to keeping small files
                    if pipeline: remove_small_files = 'n'
                ...
                if len(fasta_file)==1:
                    ...
                    with open(os.path.join(fastas_folder,fasta_file)) as f:
                        first_line = f.readline().strip()
                        if ' ' in first_line:
                            print(TextColours.WARNING+"\nYour fasta headers in "+fasta_file+"contain spaces, please replace these with underscores before running seqfinder.\nYou could remove these with: for file in *.fasta; do sed -i '/^>/ s/ /_/g' $file; done"+TextColours.ENDC)
                            quit()
                        if "/" in first_line:
                            print(TextColours.WARNING+"\nYour fasta headers in "+fasta_file+"contain slashes, please replace these with underscores before running seqfinder.\nYou could remove these with: for file in *.fasta; do sed -i '/^>/ s/\//_/g' $file; done"+TextColours.ENDC)
                            quit()
                elif len(fasta_file)==0:
                    fasta_ok="not_found"
                    print(TextColours.FAIL+"Missing assembly file for: "+sample_name+TextColours.ENDC)
                else:
            ...
            if R2_ok=="found" and fasta_ok=="found":
                fastq_to_process.append([os.path.join(data_path,fastq_R1),os.path.join(data_path,fastq_R2),os.path.join(fastas_folder,fasta_file),file_size,min_trimming_length])
    ...
    if not os.path.isfile(sample_list_file):
        ...
        summary=[["R1_file","fasta_file","R1_status","R2_status","fasta_status","min_trimming_length"]]
        min_trimming_length=80
        with open(sample_list_file, 'r') as f:
            for line in f:
                line = line.strip()
                if "," not in line:
                    print(TextColours.WARNING+"Expected comma separated file but there are lines without commas in your file. Check there are no blank lines or that filepaths are not separated by something else."+ TextColours.ENDC)
                    sys.exit()
                if "~" in line:
                    print(TextColours.WARNING + "\nYour filepaths in your sample file contain a tilde (~). Please provide the full path and run again." + TextColours.ENDC)
                    sys.exit()
                if "/home/" not in line:
                    print(TextColours.WARNING + "\nYour sample file contains a blank line, headers or the full path to your files has not been provided.\n\nNo headers are required in the sample file, please see the example in " + soft_path + "\n" + TextColours.ENDC)
                    sys.exit()
                paths = line.split(',')
                for path in paths:
                    path = path.strip()
                    if not path.startswith('/home/'):
                        print(TextColours.WARNING + f"\nInvalid path detected: {path}.\nPlease ensure all paths start with '/home/' and provide the full path." + TextColours.ENDC)
                        sys.exit()
        ...
        for sample in samples:
            ...
            permitted_assembly_extensions = ('.fa', '.fna', '.fasta')
            if os.path.isfile(fasta_file) and fasta_file.endswith(permitted_assembly_extensions):
                fasta_ok="found"
                with open(os.path.join(fasta_file)) as f:
                    first_line = f.readline().strip()
                    if ' ' in first_line:
                        print(TextColours.WARNING+"\nYour fasta headers contain spaces, please replace these with underscores before running seqfinder.\nYou could remove these with: for file in *.fasta; do sed -i '/^>/ s/ /_/g' $file; done"+TextColours.ENDC)
                        quit()
            else:
                fasta_ok="not_found"
                print(TextColours.FAIL+"Missing assembly file for: "+fasta_file+TextColours.ENDC)
            if R1_ok=="found" and R2_ok=="found" and fasta_ok=="found":
                fastq_to_process.append([fastq_R1,fastq_R2,fasta_file,file_size,min_trimming_length])
...
#Copy arguments file to output folder to make it easier to debug if required
shutil.copy(arguments_file, output_path)  
...
print("\n***** There are "+str(len(fastq_to_process))+" isolates that APHASeqFinder will process against "+reference_name)
...
for sublist in fastq_to_process:
    ...
    if not match_found:
        ...
        if not matching_row.empty:
            corresponding_path = matching_row['path'].iloc[0]
            if pd.notna(corresponding_path):
                sublist.append(corresponding_path)
                match_found = True

    if not match_found:
        sublist.append("None_found")
        isolate_name = sublist[0].split(os.sep)[-1].split("_")[0]
        print(TextColours.FAIL + f"Isolate {isolate_name} failed to be designated an MLST scheme so depth normalisation will not occur.\n" + TextColours.ENDC)
...
try:
    ...
    pool.close()
    pool.join()
print(TextColours.OKBLUE+"\nFinished building reference databases\n"+ TextColours.ENDC)
...
if os.path.isfile(processed_csv_path):
    delete_files(output_path,'processed.csv')
...
#with open(os.path.join(output_path, "processed.csv"), "w") as f:
#    writer = csv.writer(f)
#    writer.writerow(["Isolate", "PID"])
try:
...
#Delete folder containing reads that had been split in half by fastx
...
'''
#Turn on for speed checking
out_file=os.path.join(output_path,out_file_prefix+"__pipeline_timings.csv")
combine_tables_of_results(output_path,'*_timings.csv',out_file)
'''
#TODO: eliminate AMR, BAM and VIR differences?
if database_type=="AMR":
...
try:
    #HACK: skip bad lines to prevent Error tokenizing data on No genes passed filter rows, which have more cols than they should
    abri_seq=pd.read_csv(out_file_abricate_seqfinder_compilation,on_bad_lines='skip')
    #abri_seq=pd.read_csv(out_file_abricate_seqfinder_compilation)
    #TODO: push this list into a database field? E.g. EFSA dict etc could have a column "warn_on_hit"?
    genes_to_check = [
        ...
        "colis-g2867_mcr5-2","betaL-g1033_OXA-48","tetra-g1931_tet-X","tetra-g2313_tet-X2",
        "tetra-g2314_tet-X3","tetra-g2315_tet-X3","tetra-g2316_tet-X4","tetra-g2374_tet-X5","tetra-g2375_tet-X6",
        "tetra-g2970_tet-X1", "amino-g0167_rmtB",
    ...
    for idx, row in abri_seq.iterrows():
        ...
        if found_genes:
            print(TextColours.FAIL+"\n Warning "+str(row['strain']) + " contains " + ', '.join(found_genes)+TextColours.ENDC)
except Exception as e:
    print("Error:", e)
...
print(TextColours.FAIL+"\n\u2764\ufe0f"+TextColours.ENDC+TextColours.OKBLUE+" Thank you for running APHA SeqFinder "+TextColours.FAIL+"\u2764\ufe0f"+TextColours.ENDC)  # Unicode for red heart
