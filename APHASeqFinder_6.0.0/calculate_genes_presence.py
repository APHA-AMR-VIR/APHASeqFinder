...
def pileup_vcf(vcf_file_name,fq_file_name,ref_name):
    print("Processing vcf file: "+vcf_file_name)
    ...
    fileIn = open(vcf_file_name, 'r', encoding="utf-8")
    vcflen=0
    for line in fileIn:
...
if len(args)<2:
    sample_folder="/home/nickduggett/seqfinder_testing/AMRDatabase_20230526_and_EnteroPLasmids_20230526_short-tetA6/20231129/4041"
    ref_fasta="/home/nickduggett/APHASeqFinder_4.1.0/references/AMR/AMRDatabase_20230526_and_EnteroPLasmids_20230526_short-tetA6.fna"
    mlst_fasta="/home/nickduggett/APHASeqFinder_4.1.0/references/mlst/ECO-MLST-MG1655-alleles.fna"
    thqual = 150
    thSnpCovProp=0.2
    thMinGC=2
else:
...
if mlst_fasta != "None_found":
    ...
    if mlst_tab:
        ...
        print("Normalising mapped depth by the mlst median coverage")
else:
    print("No MLST genes found for normalisation of mapped depth. Defaulting to 1.")
    median_cov_ref_mlst = 1
...
for i in range(len(table)):
    try:
        ...
        print(mlst_error)
        norm_value=round(table[i][3]/1,2)
        with open(sample_folder+'/mlst_error.txt', 'w', encoding="utf-8") as f:
...
out_file=fq_file_name[:-3]+"_CompareTo_"+reference_name.split(os.sep)[-1].split(".")[0]+".csv"
