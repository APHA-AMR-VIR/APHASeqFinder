...
if len(sys.argv)>1:
    df_abr = pd.read_csv(sys.argv[1])
    df_sf = pd.read_csv(sys.argv[2])
    df_original = pd.read_csv(sys.argv[3])
    efsa_dict = pd.read_csv(sys.argv[4],usecols=['id', 'antimicrobial'])
    first_line = sys.argv[5]
    abricate_version = sys.argv[6]
    mlst_fasta = sys.argv[7]
    df_deleted_sequences = pd.read_csv(sys.argv[8])
    platon_output = pd.read_csv(sys.argv[9],sep="\t")
    snp_pattern_dict=sys.argv[10]
    gene_dict=pd.read_csv(sys.argv[11])
    
else:  # just for developing code
    pd.set_option('display.max_columns', None)
    path = "/home/nickduggett/missing_isolates/AMR_with_plasmids_20250709/20260109/LREC8660/"
    isolate_name="LREC8660"
    df_abr = pd.read_csv(path+isolate_name+'.abricate')
    df_sf = pd.read_csv(path+isolate_name+'_CompareTo_AMR_with_plasmids_20250709_good_snps.csv')
    df_original= pd.read_csv(path+isolate_name+'_CompareTo_AMR_with_plasmids_20250709.csv')
    efsa_dict=pd.read_csv('/home/nickduggett/mnt/fsx-044/Bactipipes/BactiPipes_SCE3/AMR_github/APHASeqFinder_testing_area/APHASeqFinder_6.0.0/EFSA_panel/EFSA_antimcriobial_panel_dictionary_2025-07-09.csv',usecols=['id', 'antimicrobial'])
    df_deleted_sequences = pd.read_csv(path+isolate_name+'_DeletedSequences.csv')
    platon_output = pd.read_csv("/home/nickduggett/mnt/fsx-033/javi_temp/VM0533F/April_2025/SeqFinder/v6/AMR_with_plasmids_20230526/20250721/LREC9823-B/LREC9823_AMR.tsv",sep="\t")
    platon_output = platon_output.drop([0,1,2])
    ...
    snp_pattern_dict='/home/nickduggett//mnt/fsx-044/Bactipipes/BactiPipes_SCE3/AMR_github/APHASeqFinder_6.0.0/references/AMR/AMR_test_db_Chr_SNP_patterns.csv'
    gene_dict=pd.read_csv('/home/nickduggett/mnt/fsx-044/Bactipipes/BactiPipes_SCE3/AMR_github/APHASeqFinder_testing_area/APHASeqFinder_6.0.0/references/AMR/gene_name_dictionary.csv')
    with open('/home/nickduggett/mnt/fsx-044/Bactipipes/BactiPipes_SCE3/AMR_github/APHASeqFinder_testing_area/test_panel/2682_spades.fa', encoding="utf-8") as f:
        first_line = f.readline().strip()

df_sf = df_sf.drop(['warnings'], axis=1)
strain_name = df_sf['strain'].iloc[0]
'''
Need to add in check for abricate output being empty here but column order could be an option so a way to bypass could be needed
check_if_empty = df_abr.empty
if check_if_empty == True:
    new_row = {col: "No genes passed filter" for col in df_sf.columns}
    new_row['strain'] = strain_name
    df_sf = pd.DataFrame([new_row], columns=df_sf.columns)
    ...
    # Saving the output of the deleted rows
    output_filename_deleted_rows = sys.argv[2][:-4]+'_automatic_deletions.csv'
    deleted_rows.to_csv(output_filename_deleted_rows,index=False)
'''
...
filtering_rule = pd.concat(merge_ant3).drop_duplicates()
...
def clean_df_sf(df_sf, abricate_dictionary_gene, abricate_dictionary_sequence, abricate_dictionary_coverage, abricate_dictionary_identity, abricate_dictionary_location,  abricate_dictionary_coverage_map):
    ...
    return df_sf
df_sf = clean_df_sf(df_sf, abricate_dictionary_gene, abricate_dictionary_sequence, abricate_dictionary_coverage,abricate_dictionary_identity, abricate_dictionary_location, abricate_dictionary_coverage_map)


if "class" in df_sf.columns:
    df_sf['class'] = df_sf['class'].astype(str)
...
if df_sf['result_abr'].str.contains(',').any():
    ...
    def create_missing_rows(row):
        ...
        for string in missing_strings:
            ...
            if not matching_row.empty:
                ...
                print(f"Warning: No matching row found in df_sf for string '{string}'")
                continue
            for col, mapping in column_mapping.items():
                filtered_df = df_original[df_original['id'] == string]
                if not filtered_df.empty:
                    new_row[col] = filtered_df[mapping].iloc[0]
                else:
                    print(f"Warning: No match found in df_original for id '{string}'")
                    new_row[col] = None
    ...
    if new_rows:
        ...
        new_rows_df=clean_df_sf(new_rows_df, abricate_dictionary_gene, abricate_dictionary_sequence, abricate_dictionary_coverage,abricate_dictionary_identity, abricate_dictionary_location, abricate_dictionary_coverage_map)
        ...
        efsa_as_dict = efsa_dict.set_index('id')['antimicrobial'].to_dict()
...
abricate_columns=(df_sf.loc[:,['coverage_abr','identity_abr','location_abr','plasmid_abr','coverage_map_abr']])
df_sf=df_sf.iloc[:,:-5]
...
df_sf['Generated_by']=subprocess.check_output('whoami').strip().decode('utf-8')
...
# Create a dictionary that maps each strain to the set of its genes (IDs)
isolate_genes_dict = df_sf.groupby("strain")["gene"].apply(set).to_dict()

def generate_warning(row, isolate_genes):
    warnings = []
    if row["MLST_reference"] == "None_found":
        warnings.append("Normalised depth not calculated")
        
    if (row["MLST_reference"] == "senterica_achtman_2_MLST.fna" and row['id'] in ["amino-g0039_aac6-Iaa", "amino-g0082_aac6-Iy"]):
        warnings.append("Intrinsic and may not cause phenotype")
    ...
    if row["result_abr"]:
        result_ids = [x.strip() for x in row["result_abr"].split(",")]
        if row["id"] not in result_ids:
            if "PlasR" in row["result_abr"]:
    ...
    if "class" in df_sf.columns:
        if "*" in row['annotation'] and "PlasR" not in row['class']:
            warnings.append("gene has early STOP codon and may be truncated")
    elif "class" not in df_sf.columns:
        if "*" in row['annotation']:
            warnings.append("gene has early STOP codon and may be truncated")
    
    # Gene operon warnings (if using biocides and metal database)
    # Define known operons as sets of genes
    operons = {
        "znt": {"genes": {"zntR", "zntA"}, "resistance": "zinc"},
        "czc": {"genes": {"czcC", "czcB", "czcA", "czcR"}, "resistance": "zinc"},
        "znu": {"genes": {"znuA", "znuB", "znuC", "zur"}, "resistance": "zinc"},
        "zra": {"genes": {"zraR", "zraP", "zraS"}, "resistance": "zinc"},
        "sit": {"genes": {"sitA", "sitB", "sitC", "sitD"}, "resistance": "biocide and metal"},
        "hde": {"genes": {"hdeA", "hdeB"}, "resistance": "acid"},
        "mer": {"genes": {"merA", "merT", "merR", "merP"}, "resistance": "mercury"},
        "cus": {"genes": {"cusC", "cusF", "cusB", "cusA", "cusR", "cusS"}, "resistance": "copper"},
        "sil_core": {"genes": {"silC", "silB", "silA", "silP"}, "resistance": "silver"}
    }

    # Get the set of genes for the current strain
    genes_in_strain = isolate_genes.get(row["strain"], set())

    # Check if the current gene belongs to any operon and if all required genes for the operon are present in the strain
    for operon_name, info in operons.items():
        genes = info["genes"]
        resistance_type = info["resistance"]
        if row["gene"] in genes:  # Check if the current gene is part of the operon
            if genes.issubset(genes_in_strain):
                warnings.append(f"Your isolate may contain the {operon_name} operon ({', '.join(genes)}) for {resistance_type} resistance")
...
df_sf["warnings"] = df_sf.apply(lambda row: generate_warning(row, isolate_genes_dict), axis=1)
...
# List of genes that we allow mismatches between
genes = ['ant3', 'ant2', 'sul1','qnrB19','qacH']
...
#Rerunning addition of "relevant", "found snps" and "better name" columns here to cover genes readded
df_sf['snps_causing_resistance'] = df_sf.apply(lambda row: snp_patterner(row['id'],row['annotation']),axis=1)
df_sf['annotation_non_synonymous_amino_acid'] = df_sf.apply(lambda row: snp_translator(row['id'],row['annotation']),axis=1)
#df_sf['better_name'] = df_sf['id'].str.split("_").str[1] + "-" + df_sf['snps_causing_resistance']
#df_sf['better_name'] = df_sf['better_name'].str.replace("No SNPs listed for gene","")
#df_sf['better_name'] = df_sf['better_name'].str.strip("-")

#Removing redundant warning for ant3-Ia

condition = ((df_sf['id'].isin(["amino-g0118_ant3-Ia", "amino-g0117_ant3-Ia"])) &
    (df_sf['result_abr'] == "amino-g0118_ant3-Ia") &
    (df_sf['warnings'].str.contains("Abricate gene coverage is below 90%")))

# Remove the specific string from the 'warnings' column for matching rows

df_sf.loc[condition, 'warnings'] = (df_sf.loc[condition, 'warnings'].str.replace(r"(;?\s*)?Abricate gene coverage is below 90%(\s*;?)?", "", regex=True).str.replace(r";{2,}", ";", regex=True).str.strip("; "))

#Making sure we don't get abricate warning for nfsAB because they won't be found in assembly due being truncated by a stop codon.
condition_nfsAB = ((df_sf["MLST_reference"] == "senterica_achtman_2_MLST.fna") &(df_sf["id"].isin(["nitro-g3109_nfsA_Salm_Chr", "nitro-g3110_nfsB_Salm_Chr"])))
df_sf.loc[condition_nfsAB, "warnings"] = df_sf.loc[condition_nfsAB, "warnings"].str.replace("No corresponding hit in abricate;", "", regex=False).str.strip("; ")


# Conditions for automatic deletion
if 'class' in df_sf.columns:
    conditions = (
        ((df_sf['warnings'] == "Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate") & (df_sf['norm_depth'] < 0.4) & (df_sf['perc_mapped'] < 80)) |
        ((df_sf['warnings'] == "Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate") & (df_sf['norm_depth'] < 0.3) & (df_sf['perc_mapped'] < 85)) |
        ((df_sf['warnings'] == "No corresponding hit in abricate") & (df_sf['id'] == 'macro-g2383_linG')) |
        ((df_sf['warnings'] == "Percentage mapped is under 95%;No corresponding hit in abricate") & (df_sf['id'] == 'Integ-g2251_int1') & (df_sf['perc_mapped'] < 80)) |
        ((df_sf['warnings'] == "Percentage mapped is under 95%;No corresponding hit in abricate") & (df_sf['id'] == 'PlasR-p0026_IncBOKZ') & (df_sf['perc_mapped'] < 90)) |
        ((df_sf['warnings'] == "No corresponding hit in abricate") & (df_sf['class'] == "PlasR")) |
        ((df_sf['warnings'] == "No corresponding hit in abricate;Multiple genes from same family present") & (df_sf['class'] == "PlasR")) |
        ((df_sf['warnings'] == "Normalised depth is low;No corresponding hit in abricate;Multiple genes from same family present") & (df_sf['class'] == "PlasR")) |
        ((df_sf['warnings'] == "Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate;gene has early STOP codon and may be truncated")) |
        ((df_sf['warnings'].str.contains("Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate;Missing the start of the gene",na=False)& (df_sf['perc_mapped'] <80))) |
        ((df_sf['warnings'].str.contains("Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate;Missing the start of the gene",na=False)& (df_sf['norm_depth'] <0.1))) |
        ((df_sf['warnings'].str.contains("Percentage mapped is under 95%;No corresponding hit in abricate;gene has early STOP codon and may be truncated;Missing the start of the gene",na=False))) |
        ((df_sf['warnings'].str.contains("Percentage mapped is under 95%;No corresponding hit in abricate;Missing the start of the gene", na=False) & (df_sf['perc_mapped'] <80)))
    )
else:
    conditions = (
        ((df_sf['warnings'] == "Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate") & (df_sf['norm_depth'] < 0.4) & (df_sf['perc_mapped'] < 80)) |
        ((df_sf['warnings'] == "Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate") & (df_sf['norm_depth'] < 0.3) & (df_sf['perc_mapped'] < 85)) |
        ((df_sf['warnings'] == "Percentage mapped is under 95%;Normalised depth is low;No corresponding hit in abricate;gene has early STOP codon and may be truncated")) |
        ((df_sf['warnings'].str.startswith("Percentage mapped is under 95%;No corresponding hit in abricate;gene has early STOP codon and may be truncated;Missing the start of the gene"))) |
        ((df_sf['warnings'].str.startswith("Percentage mapped is under 95%;No corresponding hit in abricate;Missing the start of the gene") & (df_sf['perc_mapped'] < 80)))
    )

# Copy rows to be deleted to a new DataFrame so that we can look at them later if needed
deleted_rows = df_sf[conditions].copy()



# Remove rows from the original DataFrame to make it cleaner for the user
df_sf = df_sf[~conditions]


if "class" in df_sf.columns:
    insert_at = df_sf.columns.get_loc("genes_on_same_contig") + 1

    if not platon_output.empty:
        platon_output = platon_output[['ID', 'RDS']]
        merged_df = pd.merge(df_sf, platon_output, left_on='contig_abr', right_on='ID', how='left')
        merged_df['Platon_predicted_plasmid'] = merged_df.apply(
            lambda row: 'Yes' if pd.notnull(row['ID']) else ('No' if 'PlasR' not in row['class'] and 'Integ' not in row['class'] else 'NA'),
            axis=1
        )
        merged_df["Platon_RDS_likelihood"] = merged_df["RDS"].fillna("NA")
        merged_df = merged_df.drop(columns=['ID', 'RDS'])

        cols = merged_df.columns.tolist()
        for col in ['Platon_predicted_plasmid','Platon_RDS_likelihood'][::-1]:
        ...
        df_sf["Platon_predicted_plasmid"] = df_sf["class"].apply(
            lambda x: "No" if "PlasR" not in x and "Integ" not in x else "NA"
        )
        df_sf["Platon_RDS_likelihood"] = "NA"
...
#sort the dataframe alphabetically on the id column
df_sf=df_sf.sort_values(by=['id'])

#Removing and moving some of the columns
df_sf['mapped_len'] = df_sf['mapped_len'].astype(str) + '/' + df_sf['ref_len'].astype(str)

def split_annotation(row):
    parts = row.split(';')
    syn = ';'.join([p for p in parts if 'syn' in p])
    non = ';'.join([p for p in parts if 'non' in p])
    return pd.Series([syn, non], index=['syn', 'non'])

#HACK: to solve empty dataframe error
if len(df_sf) > 0:
    df_sf[['annotation_synonymous_nucleotide', 'annotation_non_synonymous_nucleotide']] = df_sf['annotation'].apply(split_annotation)
else:
    df_sf[['annotation_synonymous_nucleotide', 'annotation_non_synonymous_nucleotide']] = ""

non_index = df_sf.columns.get_loc('non') + 1

df_sf.insert(non_index,'snps_causing_resistance',df_sf.pop('snps_causing_resistance'))
df_sf.insert(non_index + 1,'annotation_synonymous_nucleotide', df_sf.pop('annotation_synonymous_nucleotide'))
df_sf.insert(non_index + 2, 'annotation_non_synonymous_nucleotide', df_sf.pop('annotation_non_synonymous_nucleotide'))
df_sf.insert(non_index + 3, 'annotation_non_synonymous_amino_acid', df_sf.pop('annotation_non_synonymous_amino_acid'))
if 'EFSA_dict' in df_sf.columns:
    id_to_gene = dict(zip(gene_dict['id'], gene_dict['gene']))
    df_sf['gene'] = df_sf['id'].map(id_to_gene).fillna(df_sf['gene'])
elif 'EFSA_dict' not in df_sf.columns:
    df_sf['gene']=df_sf['id'].apply(lambda x: x.split('_', 1)[1] if isinstance(x, str) and '_' in x else x)
df_sf=df_sf.drop(columns=['ref_len','Unicycler_version','SPAdes_version','snps','other','good_snps','annotation'])

#This part deals with isolates that have hits found by seqfinder which are later removed by abricate. This most commonly occurs with plasmids.
check_if_empty = df_sf.empty
if check_if_empty == True:
    new_row = {col: "No genes passed filter" for col in df_sf.columns}
    new_row['strain'] = strain_name
    df_sf = pd.DataFrame([new_row], columns=df_sf.columns)
